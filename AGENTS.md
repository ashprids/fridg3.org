...
Bundled fridgeBeats synth presets must use tagged names: `[BA]` bass, `[FX]` effects, `[LD]` leads, `[PD]` pads, `[PL]` plucks, `[SQ]` sequences when the synth supports sequencing, and `[SY]` general synth patches. Production synths should provide 10 unique presets for each applicable type.
