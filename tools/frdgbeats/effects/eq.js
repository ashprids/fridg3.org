(function () {
    ...
    function makeGraph(api) {
        ...
        fill.classList.add('fdgb-eq-fill');
        const curve = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        curve.classList.add('fdgb-eq-curve');
        ...
        readout.className = 'fdgb-eq-readout';
        const controls = document.createElement('details');
        controls.className = 'fdgb-eq-controls';
        ...
        bandPicker.className = 'fdgb-eq-band-picker';
        const sliderGrid = document.createElement('div');
        sliderGrid.className = 'fdgb-eq-slider-grid';
        ...
        [freqSlider, gainSlider].forEach(slider => {
            slider.type = 'range';
            slider.className = 'fdgb-eq-slider';
        });
        freqSlider.min = '0';
        freqSlider.max = '1000';
        freqSlider.step = '1';
        ...
        bands.forEach(band => {
            ...
            group.classList.add('fdgb-eq-handle');
            ...
            button.type = 'button';
            button.className = 'fdgb-eq-band-button';
...
.fdgb-eq-handle circle {
    fill: var(--band);
    stroke: rgba(0,0,0,0.82);
    stroke-width: 3;
    filter: drop-shadow(0 0 7px var(--band));
}
.fdgb-eq-handle.is-active circle {
    stroke: #fff;
}
.fdgb-eq-handle text {
    fill: #020202;
...
.fdgb-eq-slider-grid span {
    color: rgba(191, 255, 251, 0.62);
    text-align: right;
}
.fdgb-eq-slider-grid b {
    color: #bffffb;
    font-weight: normal;
    text-align: right;
}
...
.fdgb-eq-slider:focus {
    outline: none;
    box-shadow: 0 0 0 2px rgba(134, 211, 207, 0.2);
}
.fdgb-eq-slider::-webkit-slider-thumb {
    -webkit-appearance: none;
    appearance: none;
    width: 16px;
    height: 16px;
...
.fdgb-eq-slider::-moz-range-track {
    height: 6px;
    border: 1px solid rgba(134, 211, 207, 0.34);
    border-radius: 999px;
    background:
...
.fdgb-eq-slider::-moz-range-thumb {
    width: 16px;
    height: 16px;
    border: 2px solid #020202;
    border-radius: 4px;
