(function () {
    ...
    function makeVoiceControl(param, settings, setParam) {
        ...
        wrap.className = 'fdgb-chorus-voice';
        wrap.style.setProperty('--value', String(ratio));
        const wave = document.createElement('span');
        wave.className = 'fdgb-chorus-wave';
...
.fdgb-chorus-title span:last-child { color: rgba(185,221,255,0.58); }
.fdgb-chorus-voices {
    display: grid;
...
.fdgb-chorus-voice span,
.fdgb-chorus-voice strong {
    max-width: 100%;
    overflow: hidden;
...
.fdgb-chorus-voice strong { color: #76beff; }
.fdgb-chorus-voice input {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
...
@media (max-width: 760px) {
    .fdgb-chorus-voices { grid-template-columns: repeat(2, minmax(74px, 1fr)); }
}
`,
