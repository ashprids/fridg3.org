(function () {
    ...
    function makeSlider(param, settings, setParam) {
        ...
        wrap.className = 'fdgb-pitch-control';

        const top = document.createElement('span');
        top.className = 'fdgb-pitch-control-top';
        const label = document.createElement('span');
        label.textContent = param.label || param.id;
        const readout = document.createElement('span');
        readout.className = 'fdgb-pitch-readout';
...
.fdgb-pitch-title span:last-child {
    color: #ff8fbd;
}
.fdgb-pitch-grid {
    display: grid;
...
.fdgb-pitch-control input[type="range"] {
    --value: 0.5;
    width: 100%;
    height: 12px;
    appearance: none;
...
.fdgb-pitch-control input[type="range"]::-webkit-slider-thumb {
    appearance: none;
    width: 20px;
    height: 20px;
    border-radius: 50%;
...
.fdgb-pitch-control input[type="range"]::-moz-range-thumb {
    width: 18px;
    height: 18px;
    border-radius: 50%;
    border: 2px solid #101820;
...
`,
        ...
        renderGui(api) {
            ...
            root.className = 'fdgb-pitch-box';
            const title = document.createElement('div');
            title.className = 'fdgb-pitch-title';
            title.innerHTML = '<span>instrument pitch</span><span>pre engine</span>';
            const grid = document.createElement('div');
            grid.className = 'fdgb-pitch-grid';
