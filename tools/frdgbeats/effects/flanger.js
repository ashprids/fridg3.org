(function () {
    ...
    function makeWheel(param, settings, setParam) {
        ...
        wrap.className = 'fdgb-flanger-wheel';
        wrap.style.setProperty('--value', String(ratio));
        const disc = document.createElement('span');
        disc.className = 'fdgb-flanger-disc';
        const name = document.createElement('span');
        name.className = 'fdgb-flanger-name';
        name.textContent = param.label || param.id;
        const readout = document.createElement('span');
        readout.className = 'fdgb-flanger-readout';
...
.fdgb-flanger-title span:last-child { color: rgba(170, 255, 206, 0.58); }
.fdgb-flanger-bank {
    display: grid;
...
.fdgb-flanger-readout { color: #6decb0; }
.fdgb-flanger-wheel input {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
...
@media (max-width: 760px) {
    .fdgb-flanger-bank { grid-template-columns: repeat(2, minmax(72px, 1fr)); }
}
`,
        ...
        renderGui(api) {
            ...
            root.className = 'fdgb-flanger-panel';
            const title = document.createElement('div');
            title.className = 'fdgb-flanger-title';
            title.innerHTML = '<span>flanger</span><span>comb sweep</span>';
            const bank = document.createElement('div');
            bank.className = 'fdgb-flanger-bank';
