(function () {
    ...
    function makeControl(param, settings, setParam) {
        if (param.type === 'select') {
            ...
            wrap.className = 'fdgb-phaser-node fdgb-phaser-select-node';
            const label = document.createElement('span');
            label.textContent = param.label || param.id;
            const select = document.createElement('select');
            select.className = 'fdgb-phaser-select';
        ...
        const wrap = document.createElement('label');
        wrap.className = 'fdgb-phaser-node';
        wrap.style.setProperty('--value', String(ratio));
        const orb = document.createElement('span');
        orb.className = 'fdgb-phaser-orb';
        const label = document.createElement('span');
        label.textContent = param.label || param.id;
...
.fdgb-phaser-title span:last-child { color: rgba(227, 195, 255, 0.56); }
.fdgb-phaser-map {
    display: grid;
...
.fdgb-phaser-node span,
.fdgb-phaser-node strong {
    max-width: 100%;
    overflow: hidden;
...
.fdgb-phaser-node strong { color: #ca92ff; }
.fdgb-phaser-node input {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
...
@media (max-width: 760px) {
    .fdgb-phaser-map { grid-template-columns: repeat(2, minmax(68px, 1fr)); }
}
`,
        ...
        renderGui(api) {
            ...
            root.className = 'fdgb-phaser-panel';
            const title = document.createElement('div');
            title.className = 'fdgb-phaser-title';
            title.innerHTML = '<span>phaser</span><span>allpass orbit</span>';
            const map = document.createElement('div');
            map.className = 'fdgb-phaser-map';
