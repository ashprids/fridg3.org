(function () {
    ...
    function makeFader(param, settings, setParam) {
        ...
        wrap.className = 'fdgb-reverb-fader';
        wrap.style.setProperty('--value', String(ratio));

        const name = document.createElement('span');
        name.className = 'fdgb-reverb-fader-name';
        name.textContent = param.label || param.id;

        const rail = document.createElement('span');
        rail.className = 'fdgb-reverb-rail';
        const fill = document.createElement('span');
        fill.className = 'fdgb-reverb-fill';
        const handle = document.createElement('span');
        handle.className = 'fdgb-reverb-handle';
        rail.append(fill, handle);

        const readout = document.createElement('span');
        readout.className = 'fdgb-reverb-readout';
        readout.textContent = curveText(current, param.unit);

        const input = document.createElement('input');
        input.className = 'fdgb-reverb-hidden-range';
        input.type = 'range';
...
.fdgb-reverb-title strong {
    color: #d9f6ff;
    font-family: "MainBold", monospace;
    font-size: 13px;
}
.fdgb-reverb-title span {
