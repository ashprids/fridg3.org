...
- Use unique class names prefixed with the effect id, such as `.fdgb-delay-*`.
...
```sh
node -c tools/frdgbeats/effects/my-effect.js
php -l tools/frdgbeats/effects/index.php
```

Then load `/tools/frdgbeats/`, open the mixer tab, add the effect to a channel, and check:
