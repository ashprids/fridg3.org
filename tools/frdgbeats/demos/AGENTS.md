...
```json
{
  ...
  "assets": {
    "soundfont": null,
    "soundfontUrl": "/tools/frdgbeats/soundfonts/Roland_SC-55.sf2"
  },
  "clips": {},
  "selectedId": "lead",
  "channels": []
...
- `assets.soundfontUrl`: optional bundled project-wide SoundFont URL. Use `/tools/frdgbeats/soundfonts/Roland_SC-55.sf2` for the default bundled bank, or another `.sf2` listed from `/tools/frdgbeats/soundfonts/`.
...
- For SoundFonts, prefer the bundled default Roland SC-55 at `/tools/frdgbeats/soundfonts/Roland_SC-55.sf2` unless the demo specifically needs a custom bank.
...
When `assets.soundfont` is `null`, demos may set `assets.soundfontUrl` to a bundled `.sf2` file in `/tools/frdgbeats/soundfonts/`; the app loads that bank instead of embedding the file in the project JSON.
...
- `source`: `"synth"`
- `synthType`: a registered synth id from `/tools/frdgbeats/synths/`, for example `"wave-oscillator"`, `"analog-mono"`, `"chip-stack"`, or `"glass-fm"`.
...
- Or set `sampleUrl` to a bundled file from `/tools/frdgbeats/samples/` and `sampleSource` to `"bundled"`.
...
Use effect `type` ids from `/tools/frdgbeats/effects/`.
...
```sh
php -l tools/frdgbeats/demos/index.php
```

Then serve the site and open `/tools/frdgbeats/`:
