# frdgBeats Preset Authoring Guide

This folder stores starter project presets for frdgBeats. Presets must end in `.frdgbeats`; `index.php` lists them in the in-app new project menu after the built-in `blank project` option.

Presets are JSON frdgBeats projects. Keep them lightweight and focused on starting points, not full demo songs. For full showcases, use `/tools/frdgbeats/demos/`.
...
- Prefer bundled SoundFonts via `assets.soundfontUrl`, usually `/tools/frdgbeats/soundfonts/Roland_SC-55.sf2`.
...
- `synth`: use a `synthType` from `/tools/frdgbeats/synths/` when the preset needs a more specific sound.
...
```bash
php -l tools/frdgbeats/presets/index.php
node -c tools/frdgbeats/frdgbeats.js
```

Then load `/tools/frdgbeats/`, open the new project menu, and confirm the preset appears and loads without console errors.
