...
- Use unique class names prefixed with the synth id, such as `.fdgb-glass-fm-*`.
...
```sh
node -c tools/frdgbeats/synths/my-synth.js
php -l tools/frdgbeats/synths/index.php
node -c tools/frdgbeats/frdgbeats.js
```

Then load `/tools/frdgbeats/`, make a channel source `synth`, pick your synth, open the synth tab, and check:
