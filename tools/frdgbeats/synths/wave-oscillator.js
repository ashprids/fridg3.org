(function () {
    ...
    function makeControl(param, api, onUpdate) {
        ...
        wrap.className = 'fdgb-wave-osc-control';
        const name = document.createElement('span');
        name.textContent = param.label || param.id;
        if (param.type === 'select') {
            const select = document.createElement('select');
            select.className = 'fdgb-select';
        ...
        meter.className = 'fdgb-wave-osc-meter';
...
.fdgb-wave-osc-head span:last-child {
...
.fdgb-wave-osc-canvas {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
}
.fdgb-wave-osc-controls {
    display: grid;
    grid-template-columns: repeat(5, minmax(74px, 1fr));
    gap: 8px;
}
.fdgb-wave-osc-control {
    --value: 0.5;
    position: relative;
    display: grid;
    gap: 6px;
...
.fdgb-wave-osc-control span,
.fdgb-wave-osc-control strong {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
...
.fdgb-wave-osc-control strong { color: #c88420; }
.fdgb-wave-osc-meter {
    height: 34px;
...
.fdgb-wave-osc-control input[type="range"] {
    position: absolute;
    inset: 0;
    width: 100%;
    height: 100%;
...
@media (max-width: 760px) {
    .fdgb-wave-osc-controls { grid-template-columns: repeat(2, minmax(74px, 1fr)); }
}
`,
        ...
        renderGui(api) {
            ...
            controls.className = 'fdgb-wave-osc-controls';
