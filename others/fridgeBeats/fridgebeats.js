(function () {
    const root = document.getElementById('fridgebeats-daw');
    ...
    const STORAGE_KEY = 'fridgebeats-project-v1';
    const DEFAULT_SOUNDFONT_URL = '/others/fridgeBeats/soundfonts/Roland_SC-55.sf2';
    const SOUNDFONT_LIST_URL = '/others/fridgeBeats/soundfonts/index.php';
    const SAMPLE_LIST_URL = '/others/fridgeBeats/samples/index.php';
    const DEMO_LIST_URL = '/others/fridgeBeats/demos/index.php';
    const PRESET_LIST_URL = '/others/fridgeBeats/presets/index.php';
    const EFFECT_LIST_URL = '/others/fridgeBeats/effects/index.php';
    const SYNTH_LIST_URL = '/others/fridgeBeats/synths/index.php';
    const RUBBERBAND_WORKER_URL = '/others/fridgeBeats/vendor/rubberband-worker.js';
    ...
    window.fridgeBeatsEffects = window.fridgeBeatsEffects || {};
    window.fridgeBeatsEffects.register = registerEffect;
    ...
    window.fridgeBeatsSynths = window.fridgeBeatsSynths || {};
    window.fridgeBeatsSynths.register = registerSynth;
    ...
    function safeFilename(name, extension) {
        return (name || 'fridgebeats').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') + extension;
    }
    ...
    async function importMidiFile(file) {
        ...
        await setExportProgress(progress, 58, 'building fridgeBeats project');
    ...
    function renderDemoMenu(items = [], failed = false) {
        ...
        if (failed) {
            els.demosMenu.append(menuNotice('fa-triangle-exclamation', 'demo list unavailable', 'could not read /others/fridgeBeats/demos'));
            return;
        }
        if (!items.length) {
            els.demosMenu.append(menuNotice('fa-folder-open', 'no demos found', 'add .frdgbeats files to /others/fridgeBeats/demos'));
            return;
        }
        items.forEach(item => {
            const button = document.createElement('button');
    ...
    function renderPresetMenu(items = [], failed = false) {
        ...
        if (failed) {
            els.presetsMenu.append(menuNotice('fa-triangle-exclamation', 'preset list unavailable', 'could not read /others/fridgeBeats/presets'));
            return;
        }
        if (!items.length) {
            els.presetsMenu.append(menuNotice('fa-folder-open', 'no presets found', 'add .frdgbeats files to /others/fridgeBeats/presets'));
            return;
        }
        items.forEach(item => {
            const button = document.createElement('button');
    ...
    async function loadPresetList() {
        els.presetsMenu.innerHTML = '';
        els.presetsMenu.append(menuNotice('fa-spinner', 'loading presets', 'checking /others/fridgeBeats/presets'));
        try {
    ...
    async function loadDemoList() {
        els.demosMenu.innerHTML = '';
        els.demosMenu.append(menuNotice('fa-spinner', 'loading demos', 'checking /others/fridgeBeats/demos'));
        try {
    ...
    function renderSoundfontMenu(items = soundfontCatalog, failed = false) {
        ...
        if (failed) {
            els.soundfontMenu.append(menuNotice('fa-triangle-exclamation', 'soundfont list unavailable', 'could not read /others/fridgeBeats/soundfonts'));
            return;
        }
        if (!items.length) {
            els.soundfontMenu.append(menuNotice('fa-folder-open', 'no soundfonts found', 'add .sf2 files to /others/fridgeBeats/soundfonts'));
            return;
        }
        items.forEach(item => {
            const button = document.createElement('button');
