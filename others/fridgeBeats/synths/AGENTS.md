# fridgeBeats Synth Authoring Guide

This folder contains modular Web Audio synth instruments loaded by fridgeBeats. Think tiny browser VSTs: each `.js` file registers one playable instrument and can expose a custom graphical panel in the synth tab.
...
- Call `window.fridgeBeatsSynths.register(...)` exactly once.
...
- `css`: optional synth-specific CSS. fridgeBeats injects it once when the synth registers.
- `renderGui(api)`: optional custom synth UI renderer. Return a DOM node. If omitted, fridgeBeats renders default param controls.
...
```sh
node -c others/fridgeBeats/synths/my-synth.js
php -l others/fridgeBeats/synths/index.php
node -c others/fridgeBeats/fridgebeats.js
```

Then load `/others/fridgeBeats/`, make a channel source `synth`, pick your synth, open the synth tab, and check:
