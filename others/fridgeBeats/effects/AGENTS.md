# fridgeBeats Effect Authoring Guide

This folder contains modular Web Audio effects loaded by fridgeBeats. Each effect should have its own graphical design.
...
- Call `window.fridgeBeatsEffects.register(...)` exactly once.
...
- `css`: optional string of effect-specific CSS. fridgeBeats injects it once when the effect registers.
- `renderGui(api)`: optional custom mixer UI renderer. Return a DOM node. If omitted, fridgeBeats renders the normal param controls.
...
- Do not connect directly to `context.destination`; fridgeBeats connects your effect output into the channel bus.
- For dry/wet effects, create explicit dry and wet gain paths.
- Clamp feedback below `1` to avoid runaway sound.
- If you create oscillators or other scheduled sources, call `.start()` in `create` and include them in `nodes` so fridgeBeats can stop/disconnect them.
...
```sh
node -c others/fridgeBeats/effects/my-effect.js
php -l others/fridgeBeats/effects/index.php
```

Then load `/others/fridgeBeats/`, open the mixer tab, add the effect to a channel, and check:
