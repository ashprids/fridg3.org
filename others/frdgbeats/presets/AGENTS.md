...
Presets are JSON frdgBeats projects. Keep them lightweight and focused on starting points, not full demo songs. For full showcases, use `/others/frdgbeats/demos/`.
...
- Prefer bundled SoundFonts via `assets.soundfontUrl`, usually `/others/frdgbeats/soundfonts/Roland_SC-55.sf2`.
...
- `synth`: use a `synthType` from `/others/frdgbeats/synths/` when the preset needs a more specific sound.
...
```bash
php -l others/frdgbeats/presets/index.php
node -c others/frdgbeats/frdgbeats.js
```

Then load `/others/frdgbeats/`, open the new project menu, and confirm the preset appears and loads without console errors.
