...
- currently exposes `theme`, `glowIntensity`, `colors`, `mobileFriendlyView`, `onekoEnabled`, and `reduceMotion`; `colors` is honored by `classic` for the full palette and by `ambercrt`/`CRT` for the single `links` phosphor color
